...
import { AIRCRAFT_POLL_MS } from "@/lib/constants";

interface AircraftPollingResult {
  geojson: GeoJSON.FeatureCollection<GeoJSON.Point> | null;
  count: number;
  loading: boolean;
}

export function useAircraftPolling(enabled: boolean): AircraftPollingResult {
  ...
  useEffect(() => {
    ...
    loadData();
    const iv = setInterval(loadData, AIRCRAFT_POLL_MS);
