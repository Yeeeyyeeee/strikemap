...
export default memo(function InterceptDashboard({ incidents }: InterceptDashboardProps) {
  ...
  const stats = useMemo(() => {
    ...
    for (const inc of withIntercept) {
      ...
      if (inc.intercept_success === false) {
        totalLeaked += inc.missiles_fired
          ? inc.missiles_fired - (inc.missiles_intercepted || 0)
          : 1;
      }
    }
  ...
  const systems = useMemo(() => {
    const map = new Map<
      string,
      { intercepted: number; missed: number; unknown: number; pFired: number; pIntercepted: number }
    >();
    for (const inc of withIntercept) {
    ...
    return Array.from(map.entries())
      ...
      .sort(
        (a, b) => b.intercepted + b.missed + b.unknown - (a.intercepted + a.missed + a.unknown)
      );
  }, [withIntercept]);
  ...
  const sideStats = useMemo(() => {
    ...
    return {
      iran: {
        ...iranDef,
        rate:
          iranDef.intercepted + iranDef.missed > 0
            ? Math.round((iranDef.intercepted / (iranDef.intercepted + iranDef.missed)) * 100)
            : 0,
      },
      west: {
        ...westDef,
        rate:
          westDef.intercepted + westDef.missed > 0
            ? Math.round((westDef.intercepted / (westDef.intercepted + westDef.missed)) * 100)
            : 0,
      },
    };
  }, [withIntercept]);
      ...
      <div className="h-full overflow-y-auto bg-[#0d0d0d]">
        <div className="max-w-5xl mx-auto px-4 py-12 flex flex-col items-center justify-center gap-4">
          ...
          <p className="text-sm text-neutral-600 text-center max-w-md">
            Interception events will appear here as defense system engagements are detected from
            OSINT feeds.
          </p>
        </div>
      </div>
      ...
      <div className="max-w-6xl mx-auto px-4 py-6 space-y-6">
        ...
        <section>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            {systems.map((sys) => (
              <div key={sys.name} className="bg-[#1a1a1a] border border-[#2a2a2a] rounded-lg p-4">
                ...
                <div className="flex items-center gap-4 text-[11px] mb-2">
                  <span className="text-green-400">&#x2713; {sys.intercepted}</span>
                  <span className="text-red-400">&#x2717; {sys.missed}</span>
                  <span className="text-neutral-500">? {sys.unknown}</span>
                </div>
        ...
        {(sideStats.iran.total > 0 || sideStats.west.total > 0) && (
          <section>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              <SideCard label="Israel / US Defense" stats={sideStats.west} color="#3b82f6" />
              <SideCard label="Iran Defense" stats={sideStats.iran} color="#ef4444" />
            </div>
          </section>
        )}
        ...
        <section>
          ...
          <div className="bg-[#1a1a1a] border border-[#2a2a2a] rounded-lg overflow-hidden">
            <div
              className="hidden md:grid grid-cols-[140px_120px_120px_1fr_1fr] gap-2 px-4 py-2 border-b border-[#2a2a2a] text-[9px] text-neutral-600 uppercase tracking-wider"
              style={{ fontFamily: "JetBrains Mono, monospace" }}
            >
              <span>Time</span>
              <span>System</span>
              ...
              {engagements.map((inc) => (
                ...
                >
                  ...
                  <span
                    ...
                    style={{ color: SYSTEM_COLORS[inc.intercepted_by!] || "#999" }}
                  >
                    {inc.intercepted_by}
                  </span>
        ...
        <p className="text-[10px] text-neutral-600 text-center pb-4">
          Interception data based on initial reports and may be revised as verified information
          becomes available.
        </p>
      </div>
    </div>
...
function SideCard({
  ...
  return (
    <div className="bg-[#1a1a1a] border border-[#2a2a2a] rounded-lg p-4">
      ...
      <div className="text-[10px] text-neutral-600 mt-2">{stats.total} engagements</div>
    </div>
  );
}
