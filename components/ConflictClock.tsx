...
export default memo(function ConflictClock({
  incidents,
  lastIranStrikeAt = 0,
  lastUSStrikeAt = 0,
  lastIsraelStrikeAt = 0,
}: ConflictClockProps) {
  ...
  const timestamps = useMemo(
    () => ({
      iran:
        lastIranStrikeAt > 0
          ? lastIranStrikeAt
          : deriveTimestamp(incidents, (i) => i.side === "iran"),
      us: lastUSStrikeAt > 0 ? lastUSStrikeAt : deriveTimestamp(incidents, isUSSide),
      israel:
        lastIsraelStrikeAt > 0 ? lastIsraelStrikeAt : deriveTimestamp(incidents, isIsraelSide),
    }),
  ...
  useEffect(() => {
    const update = () => {
      const now = Date.now();
      if (iranRef.current)
        iranRef.current.textContent =
          timestamps.iran > 0 ? formatElapsed(now - timestamps.iran) : "—";
      if (usRef.current)
        usRef.current.textContent = timestamps.us > 0 ? formatElapsed(now - timestamps.us) : "—";
      if (israelRef.current)
        israelRef.current.textContent =
          timestamps.israel > 0 ? formatElapsed(now - timestamps.israel) : "—";
    };
    update();
