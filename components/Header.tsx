...
function Dropdown({
  ...
  return (
    <div
      ref={ref}
      className="absolute top-full left-0 mt-1 z-50 bg-[#1a1a1a] border border-[#2a2a2a] rounded-lg p-1 shadow-lg min-w-[140px]"
    >
      {children}
    </div>
  );
...
const TABS: { path: string; label: string; activeClass: string }[] = [
  { path: "/", label: "strikes", activeClass: "bg-neutral-700 text-white" },
  {
    path: "/report",
    label: "Briefing",
    activeClass: "bg-yellow-500/20 text-yellow-400 border border-yellow-500/30",
  },
  {
    path: "/satellite",
    label: "Satellite",
...
const ANALYTICS_TABS: { path: string; label: string; activeClass: string }[] = [
  {
    path: "/stats",
    label: "stats",
    activeClass: "bg-green-500/20 text-green-400 border border-green-500/30",
  },
  {
    path: "/leadership",
    label: "leadership",
...
const RECON_TABS: { path: string; label: string; activeClass: string }[] = [
  {
    path: "/airspace",
    label: "Airspace",
    activeClass: "bg-sky-500/20 text-sky-400 border border-sky-500/30",
  },
  {
    path: "/heatmap",
    label: "Heatmap",
...
const MILITARY_TABS: { path: string; label: string; activeClass: string }[] = [
  {
    path: "/weapons",
    label: "weapons",
    activeClass: "bg-purple-500/20 text-purple-400 border border-purple-500/30",
  },
  {
    path: "/killchain",
    label: "killchain",
    activeClass: "bg-cyan-500/20 text-cyan-400 border border-cyan-500/30",
  },
];
...
export default memo(function Header({
  ...
  chatOpen = false,
  onToggleChat,
  onToggleSuggestions: _onToggleSuggestions,
  onToggleChanges: _onToggleChanges,
  hasUnreadChat = false,
  activeUsers = 0,
  ...
  const strikeLabel =
    viewMode === "iran" ? t("iranian") : viewMode === "us_israel" ? t("us_israel") : t("all");
            ...
            <div className="flex items-center gap-0.5 md:gap-2 ml-auto md:ml-2">
                  ...
                  <Dropdown open={volumeOpen} onClose={() => setVolumeOpen(false)}>
                    <div className="px-3 py-2 min-w-[160px]">
                      <div className="flex items-center justify-between mb-2">
                        <span
                          className="text-[9px] font-bold text-neutral-500 uppercase tracking-wider"
                          style={{ fontFamily: "JetBrains Mono, monospace" }}
                        >
                          Volume
                        </span>
                      ...
                      <div className="flex items-center gap-2">
                        <svg
                          className="w-3 h-3 text-neutral-600 shrink-0"
                          viewBox="0 0 24 24"
                          fill="none"
                          stroke="currentColor"
                          strokeWidth="2"
              ...
              {onToggleSettings && (
                <button
                  ...
                  className={`transition-colors p-1.5 md:p-1 ${settingsOpen ? "text-red-400" : "text-neutral-500 hover:text-neutral-300"}`}
                  title="Time Filter"
                >
                  <svg
                    className="w-5 h-5 md:w-4 md:h-4"
              ...
              {onToggleChat && (
                <button
                  ...
                  className={`relative transition-colors p-1.5 md:p-1 ${chatOpen ? "text-red-400" : "text-neutral-500 hover:text-neutral-300"}`}
                  title="Live Chat"
                >
                  <svg
                    className="w-5 h-5 md:w-4 md:h-4"
          ...
          <div className="hidden md:flex items-center gap-1 bg-[#1a1a1a] border border-[#2a2a2a] rounded-lg p-0.5">
                ...
                return (
                  <div key={tab.path} className="relative">
                    ...
                    >
                      ...
                      {isHome && (
                        <svg
                          className={`w-3 h-3 transition-transform ${strikesOpen ? "rotate-180" : ""}`}
                          viewBox="0 0 12 12"
                          fill="none"
                          stroke="currentColor"
                          strokeWidth="2"
                    ...
                    {isHome && (
                      <Dropdown open={strikesOpen} onClose={() => setStrikesOpen(false)}>
                        <button
                          ...
                          className={`w-full text-left px-3 py-1.5 text-xs font-medium rounded-md transition-colors ${viewMode === "all" ? "bg-neutral-700 text-white" : "text-neutral-400 hover:text-neutral-200"}`}
                        >
                          {t("all")} ({strikes.length})
                        </button>
                        <button
                          onClick={() => {
                            onViewModeChange?.("iran");
                            setStrikesOpen(false);
                          }}
                          className={`w-full text-left px-3 py-1.5 text-xs font-medium rounded-md transition-colors ${viewMode === "iran" ? "bg-red-500/20 text-red-400" : "text-neutral-400 hover:text-neutral-200"}`}
                        >
                          {t("iranian")} ({iranCount})
                        </button>
                        <button
                          onClick={() => {
                            onViewModeChange?.("us_israel");
                            setStrikesOpen(false);
                          }}
                          className={`w-full text-left px-3 py-1.5 text-xs font-medium rounded-md transition-colors ${viewMode === "us_israel" ? "bg-blue-500/20 text-blue-400" : "text-neutral-400 hover:text-neutral-200"}`}
                        >
            ...
            <div className="relative">
              ...
              >
                Recon
                <svg
                  className={`w-3 h-3 transition-transform ${reconOpen ? "rotate-180" : ""}`}
                  viewBox="0 0 12 12"
                  fill="none"
                  stroke="currentColor"
                  strokeWidth="2"
            ...
            <div className="relative">
              ...
              >
                Military
                <svg
                  className={`w-3 h-3 transition-transform ${militaryOpen ? "rotate-180" : ""}`}
                  viewBox="0 0 12 12"
                  fill="none"
                  stroke="currentColor"
                  strokeWidth="2"
              ...
              <Dropdown open={militaryOpen} onClose={() => setMilitaryOpen(false)}>
                {MILITARY_TABS.map((sub) => {
                  ...
                  const subLabel =
                    sub.label === "killchain"
                      ? t("killchain")
                      : t(sub.label as Parameters<typeof t>[0]);
                  return (
                    <Link
                      key={sub.path}
                      href={sub.path}
